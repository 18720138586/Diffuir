...
from data.image_folder import make_dataset, make_dataset_all, make_dataset_all_text, make_dataset_3, make_dataset_5, \
    make_dataset_6, make_dataset_4, make_dataset_2
...
class AlignedDataset_all(BaseDataset):
    ...
    def __init__(self, opt, image_size, augment_flip=True, equalizeHist=True, crop_patch=True, generation=False,
                 task=None):
        ...
        self.opt = opt
        # origin----------------------------------------------------------------------------------------------------------
        self.dir_Arain = os.path.join(opt.dataroot, 'syn_rain/' + opt.phase + '/input')
        self.dir_Brain = os.path.join(opt.dataroot, 'syn_rain/' + opt.phase + '/target')
        ...
        if opt.phase == 'train':
            ...
            self.dir_Asnow = os.path.join(opt.dataroot,
                                          'Snow100K/' + opt.phase + '/Snow100K-S/synthetic')  # Snow100K-S Snow100K-L
            ...
            self.dir_Arain_syn = os.path.join(opt.dataroot,
                                              'syn_rain/' + opt.phase + '/Test2800/input')  # Rain100H, Rain100L, Test100, Test1200,
            self.dir_Brain_syn = os.path.join(opt.dataroot, 'syn_rain/' + opt.phase + '/Test2800/target')  # Test2800
            self.dir_Ablur = os.path.join(opt.dataroot,
                                          'Deblur/' + opt.phase + '/GoPro/input')  # GoPro, HIDE,  Reblur_J, Reblur_R
        ...
        if task == 'light':
            ...
            self.A_paths = sorted(
                make_dataset_4(self.dir_Arain_syn, self.dir_Alsrw, self.dir_Alol, self.dir_Asnow, opt.max_dataset_size))
            self.B_paths = sorted(
                make_dataset_4(self.dir_Brain_syn, self.dir_Blsrw, self.dir_Blol, self.dir_Bsnow, opt.max_dataset_size))
        elif task == '5':
            self.A_paths = sorted(
                make_dataset_5(self.dir_Arain_syn, self.dir_Alsrw, self.dir_Alol, self.dir_Asnow, self.dir_Ablur,
                               opt.max_dataset_size))
            self.B_paths = sorted(
                make_dataset_5(self.dir_Brain_syn, self.dir_Blsrw, self.dir_Blol, self.dir_Bsnow, self.dir_Bblur,
                               opt.max_dataset_size))
        elif task == '6':
            self.A_paths = sorted(
                make_dataset_6(self.dir_Arain_syn, self.dir_Alol, self.dir_Asnow, self.dir_Ablur, self.dir_Afog,
                               opt.max_dataset_size))
            self.B_paths = sorted(
                make_dataset_6(self.dir_Brain_syn, self.dir_Blol, self.dir_Bsnow, self.dir_Bblur, self.dir_Bfog,
                               opt.max_dataset_size))
        else:
